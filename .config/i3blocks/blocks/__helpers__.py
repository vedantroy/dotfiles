    ...
    def hook(exception_type, value, traceback_):
        ...
        with open(logfile, "ab+") as f:
            if action is action.ERROR:
                f.write(bytes(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]:\n", encoding='ascii'))
                f.write(bytes("".join(traceback.format_exception(exception_type, value, traceback_)), encoding='ascii'))
                ...
                if last_line != end_log_marker:
                    f.write(bytes("Log has reached max size limit -- nothing else will be printed\n", encoding='ascii'))
